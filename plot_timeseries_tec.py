...
def plot_timeseries_tec(year, month, day, vtec_gnss, doy, sys):
    """ نمایش سری‌زمانی TEC با متن فارسی-انگلیسی صحیح """

    time_tec = np.linspace(0, 24, len(vtec_gnss))  # تولید مقیاس زمانی از ۰ تا ۲۴ ساعت

    plt.figure(figsize=(10, 5))
    plt.plot(time_tec, vtec_gnss, '-', linewidth=1, color='k', label=fix_text(f"AVERAGE VTEC ({sys})"))

    plt.xlabel(fix_text("TIME (HOURS)"), fontproperties=font_farsi)
    plt.ylabel(fix_text("VTEC (TECU)"), fontproperties=font_farsi)
    plt.title(fix_text(f"TIME SERIES VTEC In Date {year}/{month}/{day}"), fontproperties=font_farsi)
